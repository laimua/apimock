...
将下方 `---` 分隔线之间的内容整段复制，粘贴给 Claude Code（工作目录 `D:\work\apimock`）执行。

---

你是独立第三方审查者。三方对齐复核（人类 / codex / Claude），不要看任何前序复核结论（人类复核、codex 复核、其他），直接读源码核实。

## 复核目标

报告：`docs/ISSUES-REVIEW-2026-06-23.md` 声称：

1. `docs/FUNCTIONAL-DEFECTS.md` 的 19 项功能缺陷已全部修复（commit `ae282c0`）
2. 仍存在 P0：slug 从不提交 + CJK 项目名生成空 slug
3. 仍存在 P1：改项目名无条件覆盖 slug
4. `tsc=0 错误` / `vitest=296 passed` / `eslint=0 errors 1 warning`

## 判断依据文档

- `docs/FUNCTIONAL-DEFECTS.md`（原始 19 项缺陷清单）
- `docs/FUNCTIONAL-DEFECTS-IMPL.md`（实现规格）
- `docs/FEATURE-AUDIT-REPORT.md`（审计报告）

## 你必须独立做的事

### A. 运行验证命令（不要信报告的数字，自己跑）
...
codex 独立复核指出，人类复核漏掉了 `docs/FEATURE-AUDIT-REPORT.md` 的另 3 个 P1。请打开源码确认这 3 项是否真的仍然存在：

1. **isActive 整数**：`src/app/api/projects/[id]/route.ts:42` GET 是否 `return success(projectList[0])` 无 `Boolean()` format；对比 `src/app/api/projects/route.ts:33-34` 的 format 函数。返回整数 1/0 还是布尔？
2. **全量拉取**：`src/app/projects/[id]/page.tsx`、`src/app/projects/[id]/endpoints/new/page.tsx`、`src/app/projects/[id]/endpoints/[endpointId]/page.tsx` 三处是否用 `projectsApi.list().then(find(p => p.id === id))` 而非 `projectsApi.get(id)`
3. **保留字拦截**：`src/app/api/projects/check-slug/route.ts` 是否只查 DB 唯一性、不拦截 `api`/`settings`/`share` 等保留字
...
### 19 项"已修复"抽查（逐项 PASS/FAIL + 源码证据）
<逐条>

### 报告遗漏项核对
1. isActive 整数: <存在/不存在 + 证据>
2. 全量拉取: <存在/不存在 + 证据>
3. 保留字拦截: <存在/不存在 + 证据>
...
- 不看任何前序复核结论（人类复核、codex 复核、其他），自己读源码
- 数字必须实测，禁止"应该是"
- 发现报告错就明确说错，不要客气
- 发现其他复核错也明确说，不要盲从
