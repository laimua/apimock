/**
 * @deprecated 历史 fallback 脚本。优先用 `pnpm db:migrate`（drizzle-kit
 * migrate，跑 drizzle/*.sql 带版本追踪）。本脚本保留用于：
 *   - 首次部署 / 新环境 drizzle/ 迁移文件未生成时
 *   - 离线环境无 drizzle-kit 时
 *
 * 支持 SQLite 和 MySQL。
 */
...
function migrateSQLite() {
  ...
  try {
    ...
    sqlite.exec(`CREATE TABLE IF NOT EXISTS responses (id TEXT PRIMARY KEY, endpoint_id TEXT NOT NULL, name TEXT, description TEXT, status_code INTEGER NOT NULL DEFAULT 200, headers TEXT DEFAULT '{}', body TEXT, content_type TEXT DEFAULT 'application/json', match_rules TEXT DEFAULT '{}', is_default INTEGER DEFAULT 0, priority INTEGER DEFAULT 0, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, FOREIGN KEY (endpoint_id) REFERENCES endpoints(id) ON DELETE CASCADE)`);
...
async function migrateMySQL() {
  ...
  try {
    ...
    await connection.query('CREATE TABLE IF NOT EXISTS responses (id VARCHAR(36) PRIMARY KEY, endpoint_id VARCHAR(36) NOT NULL, name VARCHAR(255), description TEXT, status_code BIGINT NOT NULL DEFAULT 200, headers TEXT, body LONGTEXT, content_type VARCHAR(100) DEFAULT "application/json", match_rules TEXT, is_default TINYINT DEFAULT 0, priority BIGINT DEFAULT 0, created_at BIGINT NOT NULL, updated_at BIGINT NOT NULL, FOREIGN KEY (endpoint_id) REFERENCES endpoints(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4');
