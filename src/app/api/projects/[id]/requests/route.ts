...
export async function GET(
  ...
  try {
    ...
    return internalError(err, 'GET /api/projects/[id]/requests');
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    // 删除前统计实际行数（DB 无关，比依赖 driver 的 changes() 返回值更稳）
    const [{ count: deleteCount }] = await db
      .select({ count: sql<number>`count(*)` })
      .from(requests)
      .where(inArray(requests.endpointId, targetEndpointIds));

    // 单条 SQL 批量删除（替代逐端点循环，避免 N 次 round trip + 中途失败留半删）
    await db.delete(requests).where(inArray(requests.endpointId, targetEndpointIds));

    return success({ deleted: deleteCount as number });
  } catch (err) {
    return internalError(err, 'DELETE /api/projects/[id]/requests');
  }
}
